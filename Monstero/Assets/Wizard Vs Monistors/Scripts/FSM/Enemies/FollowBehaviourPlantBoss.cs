...
public class FollowBehaviourPlantBoss : StateMachineBehaviour
{
    [SerializeField] private float _followSpeed = 3.5f;
    private float _alleyRange;

    private float _distance;
    private bool _fireEnemies = false;
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            if (animator.GetComponent<EnemyHealth>().curretHealth <= 50 && !_fireEnemies)
            {
                _fireEnemies = true;
                animator.SetTrigger("FireEnemies");
